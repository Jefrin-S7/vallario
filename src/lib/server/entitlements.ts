...
// Called exclusively from a verified webhook handler (never from the
// client). Idempotent per order: if entitlements for this order already
// exist, it's a no-op, so a retried webhook can never double-grant access.
